...
module.exports = function complaintRoutes(db) {
  ...
  // POST /api/complaints
  // Uses multer to accept multipart/form-data so an optional screenshot
  ...
  router.post("/", authenticate, (req, res) => {
    upload.single("attachment")(req, res, async (uploadErr) => {
      ...
      try {
        ...
        if (!category || !title || !description || !complainant_name) {
          // Clean up an already-saved file if validation fails afterward
          if (req.file) fs.unlink(req.file.path, () => {});
          return res.status(400).json({ error: "Complainant name, category, title, and description are required." });
        }
        ...
        const formattedDescription = capitalizeFirst(description.trim());
