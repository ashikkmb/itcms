...
export function ComplaintForm({ user, onSubmit, onClose }) {
  const [form, setForm]     = useState({ category: "Hardware", title: "", description: "", complainant_name: "", attachment: null });
  ...
  function handleFileChange(e) {
    ...
    if (!file) { setForm(f => ({ ...f, attachment: null })); setPreview(null); return; }
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <textarea style={{ ...S.input, height: 100, resize: "vertical", marginBottom: 18 }} placeholder="Describe the problem in detail — when it started, any error messages…" value={form.description} onChange={e => setForm(f => ({ ...f, description: capitalizeFirst(e.target.value) }))} />

      <label style={S.label}>Attach Screenshot (optional)</label>
      {!preview ? (
        <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, height: 90, border: "2px dashed #E2E8F0", borderRadius: 9, marginBottom: 24, cursor: "pointer", color: "#94A3B8", background: "#FAFCFF" }}>
          <Icons.Upload />
          <span style={{ fontSize: 12, fontWeight: 600 }}>Click to upload an image</span>
          <span style={{ fontSize: 11, color: "#CBD5E1" }}>JPG, PNG, GIF, WEBP — max 5MB</span>
        ...
        <div style={{ position: "relative", marginBottom: 24, display: "inline-block" }}>
          <img src={preview} alt="Attachment preview" style={{ maxHeight: 140, maxWidth: "100%", borderRadius: 9, border: "1px solid #E2E8F0", display: "block" }} />
          <button onClick={removeAttachment} type="button"
            style={{ position: "absolute", top: -8, right: -8, width: 26, height: 26, borderRadius: 99, background: "#DC2626", color: "white", border: "2px solid white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icons.Close />
          </button>
        </div>
      )}
    ...
    <>
      ...
      <div className="fade-in" style={{ position: "fixed", top: 0, right: 0, height: "100vh", width: 460, background: "white", zIndex: 50, boxShadow: "-8px 0 40px #0003", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <div style={{ padding: "24px 28px", flex: 1 }}>
          ...
          <DetailField label="Description"  value={complaint.description} multiline />
          {complaint.attachment_path && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "#94A3B8", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 6 }}>Attached Screenshot</div>
              <a href={complaint.attachment_path} target="_blank" rel="noopener noreferrer">
                <img src={complaint.attachment_path} alt="Complaint attachment" style={{ maxWidth: "100%", maxHeight: 220, borderRadius: 8, border: "1px solid #E2E8F0", display: "block", cursor: "zoom-in" }} />
              </a>
            </div>
          )}
